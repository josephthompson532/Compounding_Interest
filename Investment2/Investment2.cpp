...
// the Constructor which sets all member variables using an initializer list
Investment::Investment(double* t_initial_amt, double* t_monthly_deposit, double* t_annual_interest, int* t_num_years, 
                    ...
                    : m_initial_amt(t_initial_amt), m_monthly_deposit(t_monthly_deposit), m_annual_interest(t_annual_interest),
                      m_num_years(t_num_years), m_total_interest_yearly(t_total_interest_yearly), m_earned_interest_per_month(t_earned_interest_per_month),
                      m_total_per_month(t_total_per_month)
{
}
...
void Investment::calcWithoutAdditionalDeposits() {
    ...
    for (int x = 1; x < (*m_num_years * 12) + 1; x++) {
        ...
        *m_total_interest_yearly += *m_earned_interest_per_month;
        ...
        if (x%12 == 0) {
            std::cout << "  " << x << std::setw(23) << std::fixed << std::setprecision(2) << *m_initial_amt << std::setw(26) << std::fixed << std::setprecision(2) << *m_total_interest_yearly << "        "<< std::endl;
            *m_total_interest_yearly = 0;
        }
    }
}
